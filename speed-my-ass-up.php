    ...
    <div class="wrap smau-dashboard-wrap">
        ...
        <form id="smau-settings-form">
            ...
            <div class="smau-tab-pane" id="smau-tab-cwv">
                ...
                <!-- Compress My A$$ Companion & Upsell Card -->
                <?php if (defined('CMA_VERSION')): 
                    $cma_status = class_exists('CMA_Bulk_Optimizer') ? CMA_Bulk_Optimizer::instance()->get_library_status() : null;
                ?>
                    <div class="smau-card" style="border-color: rgba(16, 185, 129, 0.4); background: linear-gradient(135deg, rgba(16, 185, 129, 0.06) 0%, #111827 100%);">
                        <div class="smau-card-title" style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span class="dashicons dashicons-format-image" style="color: var(--smau-emerald);"></span>
                                Compress My A$$ — Companion Active
                            </div>
                            <span class="smau-badge smau-badge-suite">CONNECTED v<?php echo esc_html(CMA_VERSION); ?></span>
                        </div>
                        <p style="color: var(--smau-text-muted); font-size: 13px; margin: 0 0 16px;">
                            Zero-SaaS on-server image optimizer is active. LCP Turbo automatically preloads next-gen WebP assets generated by Compress My A$$.
                        </p>
                        <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
                            <?php if ($cma_status): ?>
                                <div style="font-size: 13px; color: var(--smau-text-main);">
                                    <strong>Storage Saved:</strong> <span style="color: var(--smau-emerald);"><?php echo esc_html($cma_status['total_saved_mb']); ?> MB</span> (<?php echo esc_html($cma_status['reduction_percent']); ?>%)
                                </div>
                                <div style="font-size: 13px; color: var(--smau-text-main);">
                                    <strong>Optimized:</strong> <?php echo esc_html($cma_status['optimized_images']); ?> / <?php echo esc_html($cma_status['total_images']); ?> images
                                </div>
                            <?php endif; ?>
                            <a href="<?php echo esc_url(admin_url('admin.php?page=compress-my-ass')); ?>" class="smau-btn smau-btn-emerald" style="padding: 6px 14px; font-size: 12px; text-decoration: none;">
                                Open CMA Console &rarr;
                            </a>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="smau-card" style="border-color: rgba(56, 189, 248, 0.3); background: linear-gradient(135deg, rgba(2, 132, 199, 0.08) 0%, #111827 100%);">
                        <div class="smau-card-title" style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span class="dashicons dashicons-format-image" style="color: var(--smau-cyan);"></span>
                                Pair with Compress My A$$ (Zero-SaaS Media Hyper-Optimizer)
                            </div>
                            <span class="smau-badge smau-badge-pro">SENTINEL SUITE UPSELL</span>
                        </div>
                        <p style="color: var(--smau-text-muted); font-size: 13px; margin: 0 0 14px; line-height: 1.6;">
                            Traditional image optimization tools (ShortPixel, Imagify, Smush) charge costly recurring monthly subscriptions with strict credit caps and leak customer media to third-party cloud queues. <strong>Compress My A$$ (CMA)</strong> processes 100% locally on your server with unlimited WebP/AVIF generation and non-destructive master vaulting.
                        </p>
                        <div style="display: flex; gap: 14px; align-items: center;">
                            <a href="https://securemyass.com/compress-my-ass" target="_blank" class="smau-btn smau-btn-primary" style="text-decoration: none; padding: 7px 16px; font-size: 12px;">
                                Unlock Compress My A$$ &rarr;
                            </a>
                        </div>
                    </div>
                <?php endif; ?>
